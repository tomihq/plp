...
tieneMateriaAprobada(E, M) :- estudiante(E), notas(L), member((E,M,N), L), N>=4.

eliminarAplazos([], []).
eliminarAplazos([(E,M,N) | T], [(E, M, N) | L]) :- N>=4, eliminarAplazos(T, L).
eliminarAplazos([(E, M, N) | T], [(E, M, N) | L]) :- N<4, not(tieneMateriaAprobada(E, M)),  eliminarAplazos(T, L).
eliminarAplazos([(E, M, N) | T], L) :- N<4, tieneMateriaAprobada(E, M),  eliminarAplazos(T, L).

notasEstudiante(E, [], []).
notasEstudiante(E, [(E, M, N) | T], [(E, M, N) | Res]) :- notasEstudiante(E, T, Res).
notasEstudiante(E, [(E2, M, N) | T], Res) :- E \= E2, notasEstudiante(E, T, Res).