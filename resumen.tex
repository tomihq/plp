...
\end{minipage}\]
\textbf{Propiedad}: Si $f::a \rightarrow b, \ xs::[a], ys::[a]$ entonces map f (xs ++ ys) = map f xs ++ map f ys 
...
\textbf{Propiedad Inmediata}: Si $f = g$ entonces $(\forall x :: a)(f \ x  =  g \ x)$ \\
\textbf{Principio de extensionalidad funcional}: Si $(\forall x :: a)(f \ x  =  g \ x) $ entonces $f=g$
\[\begin{minipage}[b]{0.7\textwidth}
    \includegraphics[width=\linewidth]{assets/extensionalidad_funcional.png}
\end{minipage}\]
