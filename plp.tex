...
\textbf{Good to Know: Si tenés que probar $A \ge B$ y empezás desarrollando el lado derecho y aplicás HI, tenés que decir, que como la HI te manda desde B a A entonces lo que tenías antes (B) es más chico. Se nota así: $\stackrel{HI}{\le} A$}. Después agarrarías el lado izquierdo y finalizarías diciendo $A \ge A$ y es verdadero por int, o alguna cosa así.
...
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/ext_funcional.jpg}
\end{minipage}\]
...
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/ext_funcional_pares.jpg}
\end{minipage}\]
\subsection*{Lema de Generación de Booleanos}
...
\begin{itemize}
    ...
    \item e == x = false
\end{itemize}
\subsection*{Lema de Generación sobre Listas}
...
\end{itemize}
\subsection*{Lema de Generación de Pares}
Si p :: (a, b), entonces $\exists x::a . \exists y :: b. p(x,y)$
\subsection*{Lema de Generación de Sumas}
data Either a b = Left a $\mid$ Right b \\
Si $e::Either \ a \ b$ entonces 
\begin{itemize}
    \item $\exists x::a. e = Left(x)$
    \item $\exists y::b. e = Right(y)$
\end{itemize}
...
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/tipo_habitado.jpg}
\end{minipage}\]
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/tipo_habitado_2.jpg}
\end{minipage}\]
...
\subsection*{Extensiones del Cálculo Lambda}
Voy a escribir un par de tips o cosas interesantes que encontré, y después voy a escribir algunos ejercicios como guía que justamente utilizan estos tips. \\
\textbf{Tipos y Recursión en Semántica}: En las reglas de tipado tenemos contexto, en las reglas de semántica, no. Hay una \textbf{pequeña excepción}, y es, cuando necesitamos de antemano conocer como dato el tipo de salida de una función a la hora de aplicar la regla de semántica. 
1.
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/tipos_semantica.jpg}
\end{minipage}\]
\textbf{La excepción de hablar de tipos acá, en este caso particular en semántica, viene a que Nil como es un tipo anotado y ser inicialmente $Nil_{\tau}$ al aplicarle el map, necesitamos saber en qué se convierte ese $\tau$. En el caso del BIN no pasa porque no tiene un tipo anotado.} \\

2. Tenemos que modelar colas bidireccionales y se nos pide realizar las siguientes operaciones: proximo(M) y desencolar(M) con los siguientes valores: $<>_{\sigma}, V1 \cdot V2$ donde V1 es la cola de la lista y V2 es el último elemento agregado. \\
Como proximo(M) y desencolar(M) son funciones que laburan con el \textbf{primer elemento de la cola} y no el último, tenemos que hacer recursión sobre las reglas hasta llegar al caso vacío o al caso que tiene al menos un elemento. \\
El problema, es que, si no lo explícitamos bien perdemos determinismo. ¿Por qué? \\
proximo($V1 \cdot V2$) $\rightarrow$ proximo($V2$). En este caso estaríamos haciendo recursión tantas veces sea necesario para llegar al primer elemento encolado pero ¿qué pasa con si V1 es vacío?, entonces agregamos proximo($<>_{\sigma} \cdot V2$) $\rightarrow$ V2 pero hay un problema. Tenemos dos posibles reglas para aplicar (porque no sabemos que es V1 en proximo($V1 \cdot V2$)) por lo tanto perderíamos determinismo. ¿Cómo podríamos recuperarlo? Haciendo que la primera regla que usamos sea proximo($(V1 \cdot V2) \cdot V3$) $\rightarrow$ proximo($V1 \cdot V2$)
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/dequeue.jpg}
\end{minipage}\]
\textbf{A nivel enunciado, para notar esto de $(V1 \cdot V2) \cdot V3$ te lo mencionan como: "puede ser necesario mirar más de un nivel de un término para saber a qué reduce"} \\
3.Añadir la función de map para listas. 
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/map_tipado.jpg}
\end{minipage}\]
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/map_congruencia.jpg}
\end{minipage}\]
\subsection*{Valores}
Los valores son los términos que ya están completamente evaluados y no se les puede aplicar ninguna regla de evaluación adicional. En una extensión del cálculo lambda, los valores se definen inductivamente según la estructura del tipo
\subsubsection*{Valores en Árboles Binarios}
$\sigma ::= ... \mid AB_{\sigma}$ \\\
$M = ::= ... \mid Nil_{\sigma} \mid Bin(M, N, O)_{\sigma} \mid esRaiz(M) \mid esNil(M) \mid $ case M of Nil $\rightarrow$ N; Bin(i, r, d) $\rightarrow$ O \\
De acá los valores de mi nueva extensión serían: $Nil_{\sigma}$ y $Bin(V, V, V)$. ¿Por qué case, esRaiz y esNil no? Porque no nos aportan ningun posible valor nuevo. \\
Si reducimos raiz(M) va a terminar siendo, o bien un Nil o un Bin(V1, V2, V3) $\rightarrow$ V2 \\
Si reducimos esNil(M) va a terminar siendo, o bien true o false que opera en base a un Nil o un Bin(V, V, V). \\
\textbf{Importante}: ¿Qué pasa si hacemos \text{raiz(Nil)}? Explota (perdemos progreso, porque no puede reducirse más y no es un valor). En estos casos particulares que no está definido, no haríamos una regla de tipado para ese caso, y, si tenemos que hacer una macro, en el caso nil devolveríamos un $\bot$ del tipo de salida del case. 
\subsubsection*{Unión Disjunta}
$\sigma ::= ... \mid \sigma + \sigma$ \\
$M ::= ... \mid left_{\sigma}(M) \mid right_{\sigma}(M) \mid$ case M of $left(x) \rightarrow M \mid right(y) \rightarrow M$ \\
De acá, los valores de mi nueva extensión que representan a mi tipo son: $V ::= ... \mid left_{\sigma}(V) \mid right_{\sigma}(V)$. Los del case no aportan nada porque podrían devolver cualquier cosa que no tengan la forma de mis nuevos términos.
\subsection*{Reglas de Tipado}
Agregamos, al menos una por cada término nuevo que se introduce. 
\subsubsection*{Reglas de tipado con términos con ocurrencias libres}
Si tenemos una estructura del tipo condicional como un case, que devuelve un término que puede tener variables libres, entonces, en el contexto de ese caso, tenemos que añadir las variables que necesite. 
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/case_lambda.jpg}
\end{minipage}\]
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/case_lambda_ab.jpg}
\end{minipage}\]
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/recr_lambda.jpg}
\end{minipage}\]
...
\subsubsection*{Reemplazando ocurrencias libres en llamados recursivos o estructuras lazy}
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/tipado_listas.jpg}
\end{minipage}\]
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/foldr_listas.jpg}
\end{minipage}\]
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/foldr_listas_2.jpg}
\end{minipage}\]
\subsubsection*{Cantidad de Reglas de Cómputo (Semántica)}
Normalmente, agregamos una por cada término usando los valores del tipo reducido. No obstante, hay veces que un término tiene más de una regla de cómputo. \\
Ej.: Un case reduce a dos posibles cosas. Por lo tanto, tendría dos reglas de cómputo.
\subsubsection*{Reducción a un paso}
La reducción a un paso solo tiene una forma de resolver las cosas. Esto es debido a la naturaleza del determinismo con nuestras reglas en el cálculo lambda a nivel semántico. Si tuvieramos más de una regla para elegir en un paso, nuestras reglas no serían determinísticas. \\
A continuación, muestro cómo darnos cuenta de si un sistema es determinista o no.
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/determinismo_reglas.jpg}
\end{minipage}\]
\textbf{Importante}: A toda regla de congruencia se la acompaña con una regla de cómputo. No puede haber más de una de cómputo en un paso. \\
Ahora sí, adjunto reglas para un ejemplo con árboles binarios y reducción de una expresión
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/reglas_semantica_ab.jpg}
\end{minipage}\]
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/reduccion_un_paso_ab.jpg}
\end{minipage}\]
Nota: Este es el ejercicio de la dequeue. La guarda ya es un valor, por lo tanto no tenemos que reducir nada
\[\begin{minipage}[b]{1\textwidth}
    \includegraphics[width=\linewidth]{assets/reduccion_un_paso_dequeue.jpg}
\end{minipage}\]
