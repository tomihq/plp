...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item $\lambda x:\tau .$ if x then y else z true $=$ $\lambda x:\tau.$ if x then y else (z true) $=$ $\lambda x:\tau. $(if x then y else (z true)). 
    \end{itemize}
    \item fv(M): Es el conjunto de variables libres de \textbf{cada posible término M} de un sistema de tipos.
\end{itemize}
...
M \ ::= \ & x \quad &&\text{(variable)} \\
       ...
       | \ & M\ M \quad &&\text{(aplicación)} \\
       | \ & \text{true} \quad &&\text{(constante verdadera)} \\
       | \ & \text{false} \quad &&\text{(constante falsa)} \\
       | \ & \text{if } M \text{ then } M \text{ else } M \quad &&\text{(condicional)}
\end{aligned}
\]
\begin{itemize}
