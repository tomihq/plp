...
\end{minipage}\]
\section*{Programación Lógica en Prolog}
...
\subsection*{Pasaje de Logica Proposicional a Forma Clausal}
Es un algoritmo. \\
...
Ej.: $(p \lor q) \land (q \lor r)$ en Forma Clausal es $\mathcal{C} = \{\{p,q\}, \{q, r\}\}$. En este caso, esta Forma Clausal está dado por 2 Cláusulas, donde cada Claúsla tiene 2 literales.
\subsubsection*{Refutación (Método de Resolución)}
...
\textbf{Literal}: $P$
\begin{comment}
\subsubsection*{Método de Refutación (Logica Proposicional)}
Se utiliza una vez que tenemos el conjunto de cláusulas de $\mathcal{C} = \{k_{1}, \dots, k_{n}\}$. La idea es buscar una refutación, es decir una demostración de $\mathcal{C} \vdash \bot$ \\
Para poder hacer esto tenemos que hacer una conjunción de P con todos los literales y también ver la negación de P y todos los literales. La conclusión solamente posee los literales y esta conclusión se llama la resolvente de las premisas. \\
TODO: añadir imagen. \\
El algoritmo que se realiza es el siguiente:
\begin{itemize}
    \item Entrada: Un conjunto de claúsulas $\mathcal{C_{0}} = \{\mathcal{k}_{1}, \dots, \mathcal{k}_{n}\}$
    \item Salida: SAT/INSAT indicando si $\mathcal{C_{0}}$ es insatisfactible($\mathcal{C_{0}} \vdash \bot$)
    \item Algoritmo: 
    \begin{itemize}
        \item Sea $\mathcal{C}:= \mathcal{C}_{0}$. Repetir mientras sea posible.
        \item Si $\{\} \in \mathcal{C}$ devolver INSAT.
        \item Elegir dos claúsulas $\mathcal{K}, \mathcal{K'} \in \mathcal{C}$ tales que: 
        \begin{itemize}
            \item $\mathcal{K} = \{P, l_{1}, \dots, l_{n}\}$
             \item $\mathcal{K}' = \{P, l^{'}_{1}, \dots, l^{'}_{n}\}$
             \item La resolvente $\rho = \{l_{1}, \dots, l_{n}, l^{'}_{1}, \dots, l^{'}_{m}\}$
        \end{itemize}
        \item Tomar $\mathcal{C} := \mathcal{C} \cup \{\rho\}$ y volver al paso 1.
    \end{itemize}
\end{itemize}
\subsubsection*{Correción del Método de Resolución Proposicional}
\end{comment}
\subsection*{Resolución para Lógica de Primer Orden}
Hay que pasar por varias formas hasta llegar a la forma Clausal.
\subsubsection*{Forma Normal Negada (NNF)}
Solamente usa los conectivos $\{\neq, \lor, \land, \forall, \exists\}$ 
...
\subsubsection*{Prioridad de Formas Normales}
Hay tantas que puede ser un quilombo pero es así: $negada \subseteq prenexa \subseteq skolem \subseteq conjuntiva$
\subsubsection*{Refutación en Lógica Proposicional}
Dada una proposición $\sigma$ en Forma Normal Conjuntiva, la idea es calcular $\neq \sigma$. Ver si $\neq \sigma$ llegamos a una insatisfactibilidad. \\
Si llegamos a una insatisfactibilidad, entonces $\sigma$ es satisfactible. 
\subsubsection*{Corrección del método de resolución de Primer Orden}
